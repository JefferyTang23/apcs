 ...
 **/
 ...
 /*
 QCC: How do we do allMatched, can't do it. Recursively doesn't make sense because have to get to center first, not end or some other extremity
...
Proposed ALGO: arrange values for open ((,[,{) and close (),],}) (point them to the same indexes). Number opens and cancels out
when same number of close is reached. Numbers have to be in consecutive order and cannot have random inserts.
When all numbers are canceled out, true is returned.
If the close of the same number cannot be reached, return false. Stack has to start with an open.
